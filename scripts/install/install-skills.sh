...
resolve_version() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    release_page="$(download_text "https://github.com/CancerDAO/feynman-opl/releases/latest")"
...
resolve_target_dir() {
  ...
  case "$SCOPE" in
    repo)
      printf '%s/.agents/skills/feynman-opl\n' "$PWD"
      ;;
    opencode)
      printf '%s/.opencode/skills/feynman-opl\n' "$PWD"
      ;;
    codex | user)
      codex_home="${CODEX_HOME:-$HOME/.codex}"
      printf '%s/skills/feynman-opl\n' "$codex_home"
      ;;
    *)
...
if [ -z "$archive_url" ]; then
  case "$git_ref" in
    main)
      archive_url="https://github.com/CancerDAO/feynman-opl/archive/refs/heads/main.tar.gz"
      ;;
    v*)
      archive_url="https://github.com/CancerDAO/feynman-opl/archive/refs/tags/${git_ref}.tar.gz"
      ;;
  esac
fi
